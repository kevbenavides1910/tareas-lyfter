...
def student_exists(students: list[dict], full_name: str, section: str) -> bool:
...
def display_top_3(students: list[dict]) -> None:
...
def display_overall_average(students: list[dict]) -> None:
...
def display_failing_students(students: list[dict]) -> None:
    ...
    for student in students:
        failed_subjects = {
            SUBJECT_LABELS[subj]: student[subj]
            for subj in SUBJECTS
            if student[subj] < PASSING_GRADE
        }
        if failed_subjects:
